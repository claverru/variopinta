...
class ColorTests(unittest.TestCase):
    def test_gaussian_noise_has_stable_uint8_units_and_channel_policy(self) -> None:
        ...
        shifted = R.Pipeline(
            [R.GaussianNoise(mean_range=(250.0, 250.0), std_range=(0.0, 0.0))], seed=137
        )
        ...
        independent = R.Pipeline([R.GaussianNoise(std_range=(10.0, 10.0))], seed=137).compile()
        shared = R.Pipeline(
            [R.GaussianNoise(std_range=(10.0, 10.0), per_channel=False)], seed=137
        ).compile()
        ...
        with self.assertRaises(ValueError):
            R.GaussianNoise(std_range=(-1.0, -1.0))
    ...
    def test_sharpen_identity_constant_and_impulse_oracles(self) -> None:
        source = image(7, 11)
        identity = R.Pipeline(
            [R.Sharpen(blend_weight_range=(0.0, 0.0), strength_range=(5.0, 5.0))],
            seed=137,
        )
        ...
        sharpen = R.Pipeline(
            [R.Sharpen(blend_weight_range=(0.5, 0.5), strength_range=(1.0, 1.0))],
            seed=137,
        )
        ...
        with self.assertRaises(ValueError):
            R.Sharpen(blend_weight_range=(1.1, 1.1))
        with self.assertRaises(ValueError):
            R.Sharpen(strength_range=(-0.1, -0.1))

    def test_color_jitter_ranges_and_hue_are_normalized_and_explained(self) -> None:
        configured = R.ColorJitter(
        ...
        self.assertEqual(
            configured.brightness_range, tuple(float(np.float32(v)) for v in (0.8, 1.2))
        )
        self.assertEqual(configured.contrast_range, tuple(float(np.float32(v)) for v in (0.7, 1.3)))
        self.assertEqual(
            configured.saturation_range, tuple(float(np.float32(v)) for v in (0.6, 1.4))
        )
        self.assertEqual(configured.hue_range, tuple(float(np.float32(v)) for v in (-0.1, 0.1)))
    ...
    def test_color_jitter_hue_matches_reference_and_rotates_primaries(self) -> None:
        ...
        hue = R.Pipeline(
            [R.ColorJitter(**identity_ranges, hue_range=(1.0 / 3.0, 1.0 / 3.0))], seed=137
        )
        primaries = np.array(
    ...
    def test_gaussian_blur_sigma_range_matches_reference_at_arbitrary_sizes(self) -> None:
        fixed = R.GaussianBlur(kernel_size=5, sigma_range=(1.1, 1.1))
        expected_sigma = float(np.float32(1.1))
        self.assertEqual(fixed.sigma_range, (expected_sigma, expected_sigma))

        ranged = R.Pipeline([R.GaussianBlur(kernel_size=5, sigma_range=(0.6, 2.0))], seed=137)
    ...
    def test_wide_gaussian_blur_is_normalized_in_reference_and_compiled(self) -> None:
        ...
        reference = R.Pipeline(
            [R.GaussianBlur(kernel_size=101, sigma_range=(1_000_000.0, 1_000_000.0))],
            seed=137,
        )
